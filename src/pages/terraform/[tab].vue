...
function handleTabClick(tab) {
  if (tab === 'portfolio' && activeTab.value === 'project') {
    // Check if the current active tab is 'project'
    activeTab.value = tab // Activate the 'portfolio' tab
  } 
  else if (tab === 'parameter' && activeTab.value === 'portfolio') {
...
<template>
  <div>
    <VTabs v-model="activeTab" class="v-tabs-pill">
      <VTab v-for="item in tabs" :key="item.id" :value="item.tab"
        :to="{ name: 'terraform-tab', params: { tab: item.tab } }" @click="handleTabClick(item.tab)"
