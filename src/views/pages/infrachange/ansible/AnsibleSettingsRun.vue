...
const { configId, runResults } = storeToRefs(AnsibleStore)
...
<template>
    ...
    <VCol cols="12">
      <VCard title="RUN 결과">
        <VCardText class="scrollable-p">
          <p v-for="(result, name) in runResults">
  ...
  <!-- 결과값을 팝업창으로 표시 -->
  <VSnackbar v-model="addResults" v-if="addResults"  :timeout="3000"> {{ addResults }} </VSnackbar>
  <VSnackbar v-model="commitResults" v-if="commitResults" :timeout="3000"> {{ commitResults }} </VSnackbar>
  <VSnackbar v-model="pushResults" v-if="pushResults" :timeout="3000"> {{ pushResults }} </VSnackbar>
